    ...
    const [posts, setPosts] = useState<Post[]>([
        {
            ...
            content: (
                <span>
                    Acabo de terminar el módulo de <span className="text-primary font-medium">#ComunicaciónAsertiva</span>. La técnica del "Sandwich de Feedback" me parece muy útil para las revisiones trimestrales que se acercan.
                </span>
            ),
            tags: [
                            ...
                            <div className="pt-3 border-t border-gray-100 dark:border-gray-800 flex items-center justify-between">
                                <div className="flex gap-4">
                                    <button 
                                        ...
                                        className={`flex items-center gap-1.5 transition-all duration-300 hover:scale-110 active:scale-95 group ${post.isLiked ? 'text-primary' : 'text-gray-500 hover:text-primary'}`}
                                    >
                                        <span className={`material-symbols-outlined text-[20px] transition-all duration-500 ${post.isLiked ? 'fill-current material-symbols-filled scale-125 -rotate-12' : 'group-hover:scale-110'}`}>thumb_up</span>
                                        <span className="text-xs font-medium">{post.likes}</span>
                                    </button>
                                    <button 
                                        onClick={() => toggleComments(post.id)}
                                        className={`flex items-center gap-1.5 transition-all duration-300 hover:scale-110 active:scale-95 group ${post.showComments ? 'text-primary' : 'text-gray-500 hover:text-primary'}`}
                                    >
                                ...
                                <button 
                                    ...
                                    className="text-xs font-bold text-primary hover:underline transition-all active:scale-90"
                                >
                                    Responder
                                </button>
                            </div>
