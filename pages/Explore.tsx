...
const ExplorePage: React.FC = () => {
     ...
     const [pathSubFilter, setPathSubFilter] = useState('Todos'); // New sub-filter for paths
     const [expandedFaqId, setExpandedFaqId] = useState<number | null>(null);
     const { savedContent, toggleSave } = useUserStore();
     ...
     const handleToggleSave = (id: number, e: React.MouseEvent) => {
        ...
        setToastMessage(wasSaved ? "Eliminado de guardados" : "Guardado para más tarde");
        setShowToast(true);
     ...
     const pathFilters = ['Todos', 'Negocios', 'Tecnología', 'Liderazgo', 'Soft Skills']; // Filters for Rutas
     ...
     const myLearningPaths = [
        ...
        {
            id: 2,
            title: "Desarrollo Full Stack con React",
            progress: 10,
            completedCourses: 0,
            totalCourses: 8,
            image: "https://images.unsplash.com/photo-1633356122544-f134324a6cee?ixlib=rb-4.0.3&auto=format&fit=crop&w=800&q=80",
            nextCourse: "Hooks Avanzados"
        }
     ];
     ...
     const recommendedCourses = [
        {
            id: 101,
            title: "Negociación Eficaz",
            desc: "Técnicas avanzadas para cerrar acuerdos win-win.",
            rating: 4.9,
            duration: "2h 15m",
            image: "https://lh3.googleusercontent.com/aida-public/AB6AXuD3o3mGPf1c15aSUXZH3CAadspVPFP_yTlwBo8qQ7E7Ipq6U6-gpj62nOSuRn-0IKDgwhXx0Zt3CA0xzLFCaar-Hfj6jzLQ6BLhmRpFSgAnjB7nCCBIRTrrsuey5A8xi0WhC9ZlZw6flhqVmVM-ytbLVyIbo2aQwIjW3p_ygyQstnyQwaoIanPS7XNgXB0L23u3ciyXRyHrzOFsxHGutf-SExGrZZvnVVDaMGe5eZ0ELMx3-gbUeviWCCeB7jUuTW5SCf5BHSRiZQg",
            label: "Nuevo"
        },
        {
            ...
            desc: "Protege la información crítica de tu empresa.",
            rating: 4.7,
            duration: "1h 45m",
            image: "https://lh3.googleusercontent.com/aida-public/AB6AXuCd9p4cXzVJCQpA9eY6IJTzfov9vsmuSbQOcD5LPluMBuBBCJNRy4NZ_l7RmPTwHMVpAiUInjpvLYD5dl30th-tBl8AEEe6yBydT7xEBoA4ijA4o91OuIvhUqof21zGhkMOUgTgubBGvH9gqv-9zmqY3aNRhsSzAgGJlX7FLovjYGXgSY-y2QK_IW0KRZ8bxEx3aivHfxi9TLF6GzmiGWvS22RDiRkNNV-n8fCJhK_bgjZyOnfs78na-2ssOocQhovybRfOJc8LrqU",
            label: null
     ...
     const trendingItems = [
         { id: 201, title: "Inteligencia Artificial en RRHH", author: "Maria González", views: "1.2k", time: "45m", image: "https://i.pravatar.cc/150?img=20" },
         { id: 202, title: "Liderazgo en Tiempos de Crisis", author: "Carlos Ruiz", views: "950", time: "1h 10m", image: "https://i.pravatar.cc/150?img=21" },
         { id: 203, title: "Estrategias de Venta Remota", author: "Ana Silva", views: "820", time: "30m", image: "https://i.pravatar.cc/150?img=22" }
     ];
     
     const upcomingEvents = [
        {
            id: 301,
            title: "Masterclass: Liderazgo Remoto",
            date: "25",
            month: "OCT",
            time: "10:00 AM",
        ...
        {
            id: 302,
            title: "Taller: Feedback Constructivo",
            date: "28",
            month: "OCT",
            time: "02:00 PM",
            image: "https://images.unsplash.com/photo-1515168816144-b856ca00225d?ixlib=rb-4.0.3&auto=format&fit=crop&w=800&q=80",
            tag: "Taller En Vivo",
            color: "bg-emerald-600"
        },
        {
            id: 303,
            title: "Q&A con el CEO",
            date: "01",
            month: "NOV",
            time: "09:00 AM",
    ...
    const filteredMentors = mentorSubFilter === 'Todos' 
        ? mentors 
        : mentors.filter(mentor => mentor.category === mentorSubFilter);

    // Filter Logic for Courses
    const filteredCourses = courseSubFilter === 'Todos'
        ? COURSES_CATALOG
        : COURSES_CATALOG.filter(course => course.category === courseSubFilter);

    // Filter Logic for Paths
    const filteredPaths = pathSubFilter === 'Todos'
        ? LEARNING_PATHS_CATALOG
        : LEARNING_PATHS_CATALOG.filter(path => path.category === pathSubFilter);
     ...
     // Combine all saveable items including mentors
     const allItems = [...COURSES_CATALOG, ...trendingItems, ...mentors];
         ...
         <div className="relative flex h-full min-h-screen w-full flex-col bg-background-light dark:bg-background-dark shadow-xl overflow-hidden pb-24">
            <div className="px-6 pt-8 pb-2 bg-surface-light dark:bg-background-dark sticky top-0 z-20 shadow-sm border-b border-gray-100 dark:border-gray-800">
                ...
                <div className="relative w-full group mb-4">
                    ...
                    <input className="block w-full pl-10 pr-3 py-3 border-none rounded-xl bg-gray-100 dark:bg-surface-dark text-gray-900 dark:text-white focus:ring-2 focus:ring-primary placeholder-gray-500 transition-all" placeholder="¿Qué quieres aprender hoy?" type="text"/>
                </div>
            ...
            <main className="flex-1 flex flex-col gap-8 pt-6 overflow-y-auto hide-scrollbar">
                ...
                {activeFilter === 'Mentorías' ? (
                        ...
                        <div className="flex flex-col gap-4">
                            ...
                            {filteredCourses.length === 0 && (
                                <div className="flex flex-col items-center justify-center py-12 text-center opacity-60">
                                    ...
                                    <p className="text-sm font-medium text-gray-500">No se encontraron mentorías en esta categoría.</p>
                                </div>
                            )}
                        </div>
                    </div>
                        ...
                        {savedContent.length === 0 ? (
                            ...
                            <div className="flex flex-col gap-3">
                                {savedContent.map(id => {
                                    ...
                                    // Fallback for trending items if they match ID
                    ...
                    <>
                        ...
                        {/* Upcoming Events (Moved UP - High Priority) */}
                        <div>
                        ...
                        <div className="px-6">
                            <div className="flex items-center justify-between mb-4">
                                <h3 className="text-lg font-bold text-[#111318] dark:text-white">Rutas de Aprendizaje</h3>
                            </div>
                            <div className="relative w-full h-48 rounded-2xl overflow-hidden cursor-pointer group" onClick={() => navigate('/course-detail')}>
                                <div className="absolute inset-0 bg-cover bg-center transition-transform duration-700 group-hover:scale-105" style={{backgroundImage: 'url("https://images.unsplash.com/photo-1522202176988-66273c2fd55f?ixlib=rb-4.0.3&auto=format&fit=crop&w=800&q=80")'}}></div>
                                <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent"></div>
                                <div className="absolute bottom-0 left-0 p-5 w-full">
                                    <div className="flex items-center gap-2 mb-2">
                                        <span className="px-2 py-0.5 rounded bg-white/20 backdrop-blur-md text-white text-[10px] font-bold uppercase tracking-wide border border-white/10">Ruta Oficial</span>
                                        <span className="text-xs text-white/80 font-medium">4 Cursos • 12 Horas</span>
                                    </div>
                                    <h2 className="text-xl font-bold text-white leading-tight mb-1">Onboarding para Nuevos Managers</h2>
                                    <p className="text-sm text-gray-200 line-clamp-1">Domina las habilidades esenciales para liderar tu primer equipo con éxito.</p>
                                </div>
                            </div>
                        </div>

                        {/* Recommended Mentors (Carousel - Moved UP) */}
                        <div>
                            <div className="flex items-center justify-between px-6 mb-4 mt-2">
                                <h3 className="text-lg font-bold text-[#111318] dark:text-white">Mentores Sugeridos</h3>
                        ...
                        {/* Recommended Courses (Horizontal) */}
                        <div>
                            <div className="flex items-center justify-between px-6 mb-4">
                                <h3 className="text-lg font-bold text-[#111318] dark:text-white">Recomendados para ti</h3>
                                    ...
                                    return (
                                        <div key={course.id} onClick={() => navigate('/course-detail')} className="group relative flex flex-col w-[260px] shrink-0 rounded-2xl bg-white dark:bg-surface-dark shadow-sm border border-gray-100 dark:border-gray-700 cursor-pointer hover:-translate-y-1 transition-transform snap-center">
                                            <div className="h-36 w-full bg-cover bg-center relative rounded-t-2xl" style={{backgroundImage: `url("${course.image}")`}}>
                                                {course.label && (
                                                    <div className="absolute top-2 left-2 bg-white/90 dark:bg-black/70 backdrop-blur-sm px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider text-gray-800 dark:text-white shadow-sm">{course.label}</div>
                                                )}
                                                <button 
                                                    onClick={(e) => handleToggleSave(course.id, e)}
            ...
            {/* Toast Notification */}
            {showToast && (
                <div className="fixed bottom-24 left-1/2 -translate-x-1/2 z-[100] bg-slate-900 dark:bg-white text-white dark:text-slate-900 px-5 py-3 rounded-full shadow-2xl flex items-center gap-3 animate-in slide-in-from-bottom-5 duration-300 pointer-events-none border border-white/10 dark:border-gray-200">
